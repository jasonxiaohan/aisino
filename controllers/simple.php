...
class Simple extends IController
{
	...
	function send_message_mobile()
	{
		...
		$userDB = new IModel('user as u , member as m');
		$userRow = $userDB->getObj('u.username = "'.$mobile.'" and m.mobile = "'.$mobile.'" and u.id = m.user_id');
		if($userRow)
		{
			$findPasswordDB = new IModel('find_password');
			...
			{
                $smsContent = smsTemplate::login(array('{mobile_code}' => $mobile_code));

                $result = Hsms::send($mobile,$smsContent,0);
