...
class Util
{
     ...
     */
    public static function curl_message($url,$data=array())
    {
        if(empty($data)){
            $result = self::httpsrequestget($url);
        }else{
            $result = self::httpsrequest($url,$data);
        }
     ...
     * @param array $timeout 时间
     * @return string 返回的结果字符串
     */
    private static function httpsrequestget($url,$timeout=10){
        $ch = curl_init();
     ...
     * @return string 返回的结果字符串
     */
    private static function httpsrequest($url, $data = '')
    {
        $curl = curl_init();
