...
return array(
    ...
    'getCommendHot' => array(
        'query' => array(
            'name' => 'commend_goods as co',
            'join' => 'left join goods as go on co.goods_id = go.id left join cost_point 
 as cost on cost.goods_id = co.goods_id',
            'where' => 'co.commend_id = 3 and go.is_del = 0 AND go.id is not null',
            'fields' => 'go.img,go.sell_price,go.name,go.id,go.market_price,cost.point',
            'limit'=>'10',
            'order'=>'go.sort asc'
        )
    ),
 ...
 as cost on cost.goods_id = co.goods_id',
    ...
    'getGoodsByCommendgoods'=>array(
        'query'=>array(
            'name'  => 'goods as go',
            'join'  => 'left join cost_point as co on go.id = co.goods_id',
            'fields'=> 'go.id,co.`name`,co.point,go.img',
            'where' => 'go.is_del = 0 and co.point !=""',
            'order' => 'go.grade desc',
            'limit' => 12,
        )
    ),
    ...
    'getCostPointList' => array(
        'query' => array(
            ...
            'join'  => 'left join goods as go on c.goods_id = go.id left join category_extend
 as category on c.goods_id = category.goods_id',
            'where' => 'c.is_close = 0 and go.is_del = 0 and category.category_id=#category_id#',
            'order' => 'c.sort asc',
            'fields'=> 'c.*,go.img,go.sell_price,go.name as goods_name,go.market_price',
            'page'  => IReq::get('page') ? IReq::get('page') : 1,
        )
    ),
