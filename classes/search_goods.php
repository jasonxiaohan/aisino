...
class search_goods
{
	...
	public static function find($defaultWhere = '',$limit = 21,$isCondition = true)
	{
		...
		$goodsObj->fields   = 'go.id,go.name,go.sell_price,go.market_price,go.store_nums,go.img,go.sale,go.grade,go.comments,go.favorite,co.point';
		...
		// $where[] = floatval(IReq::get('min_price')) ? 'go.sell_price >= '.floatval(IReq::get('min_price')) : '';
		$where[] = floatval(IReq::get('max_price')) ? 'co.point <= '.floatval(IReq::get('max_price')) : '';
		$where[] = floatval(IReq::get('min_price')) ? 'co.point >= '.floatval(IReq::get('min_price')) : '';
