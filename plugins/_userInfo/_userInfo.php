...
class _userInfo extends pluginBase
{
	...
	public function reg()
	{
		...
		plugin::reg("isValidUserByMobile",function($data){
			list($login_info,$mobile_code) = $data;
			return self::isValidUserByMobile($login_info, $mobile_code);
		});

		//获取外部用户信息
		plugin::reg("checkAisinoUser",function($data){
			list($mobile) = $data;
			return self::checkAisinoUser($mobile);
		});
	...
	public function userLoginAct()
	{
		...
		if($userRow = plugin::trigger("isValidUserByMobile",array($login_info,$mobile_code)))
		{
	...
	public function userRegAct()
	{
		...
		$mobile_code= IFilter::act(IReq::get('mobile_code','post'));
    	$username   = IFilter::act(IReq::get('mobile','post'));
		...
		if($reg_option == 3)
		{
			...
			if($memberRow)
			{
				...
				// 同步账户积分记录
				$memberObj->setData(['point' => $userData['data']['point']]);
				$memberObj->update("user_id = ".$memberRow['user_id']);
	...
	/**
	 * @brief 通过短信验证码校验注册用户身份信息
	 * @param  [type]  $login_info  手机号码
	 * @param  [type]  $mobile_code 短信验证码
	 * @return array or false       如果合法则返回用户数据；不合法返回false
	 */
	public static function isValidUserByMobile($login_info,$mobile_code) {
		$userDB = new IModel('user as u , member as m');
		$userRow = $userDB->getObj('u.username = "'.$login_info.'" and m.mobile = "'.$login_info.'" and u.id = m.user_id');
		if($userRow)
		{
	...
	/**
	 * @brief 调用外部接口，从信商达同步用户积分数据
	 * @param  [type] $mobile 手机号码
	 * @return array or false 如果用户存在，则返回用户数据，否则返回false	 
	 */
	public static function checkAisinoUser($mobile) {		
		$result = Aisino_class::getMobile($mobile);
		return $result;
	}
